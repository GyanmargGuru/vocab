        ...
        .then(dataDict => {
            for (const [key, value] of Object.entries(dataDict)) {
                if (key.toLowerCase().includes(searchWord)) {
                    ...
                    resolve(result);
                    return;
                } else if (value["hindi_word"].some(word =>word.toLowerCase().includes(searchWord))) {
                    const result = [key, value, "hw"];
                    console.log("second", result);
                    resolve(result);
                    return;
                } else if (value["english_synonyms"].some(word =>word.toLowerCase().includes(searchWord))) {
                    const result = [key, value, "es"];
                    console.log("third", result);
                    resolve(result);
                    return;
                } else if (value["hindi_synonyms"][0].toLowerCase().includes(searchWord)) {
...
async function fetchandcreatecolumns(){
    const result = await dictionarysearch(input.value.toLowerCase());
    console.log(result);
